...
The accepted lower-shelf width, top datum, and 3.0 mm functional height remain fixed. The 3.0 mm hidden structural-thickness parameter directly drives joined internal root gussets contained within the accepted shelf and side-guide outlines. The USB-C pocket gains 0.30 mm only downward and 0.20 mm only toward the rear/depth; its edge-derived horizontal datum and 18 mm width remain unchanged. The shelf continues to carry the tablet so a standard straight USB-C connector carries no tablet load. Release gates remain in force.
