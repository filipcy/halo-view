"""HALO Dock Rev A external-print-candidate generator for Autodesk Fusion 360.

Sprint 3 retains Iteration 2 and adds small vendor fit-test coupons.
...
PARAMETERS = (
    ...
    # Sprint 3 outsourced-print coupons.
    ('coupon_length', '35 mm', 'Print coupons', 'Common compact coupon length'),
    ('coupon_center_y', '0 mm', 'Print coupons', 'Shared coupon vertical centre datum'),
    ('coupon_wall', '3 mm', 'Print coupons', 'Coupon structural wall'),
    ('coupon_height', '12 mm', 'Print coupons', 'Coupon extrusion height'),
    ('coupon_clearance_020', '0.2 mm', 'Print coupons', 'Fit coupon clearance per side'),
    ('coupon_clearance_030', '0.3 mm', 'Print coupons', 'Fit coupon clearance per side'),
    ('coupon_clearance_040', '0.4 mm', 'Print coupons', 'Fit coupon clearance per side'),
    ('coupon_slot_020', 'device_thickness + 2 * coupon_clearance_020', 'Print coupons', '0.2 mm per-side test slot'),
    ('coupon_slot_030', 'device_thickness + 2 * coupon_clearance_030', 'Print coupons', '0.3 mm per-side test slot'),
    ('coupon_slot_040', 'device_thickness + 2 * coupon_clearance_040', 'Print coupons', '0.4 mm per-side test slot'),
    ('coupon_slot_center_x', '0 mm', 'Print coupons', 'Fit-coupon slot centre'),
    ('coupon_rail_center_y', 'coupon_wall / 2', 'Print coupons', 'Fit-coupon rail centre along insertion length'),
    ('coupon_rail_center_020', 'coupon_slot_020 / 2 + coupon_wall / 2', 'Print coupons', '0.2 mm coupon rail centre magnitude'),
    ('coupon_rail_center_020_left', '-coupon_rail_center_020', 'Print coupons', '0.2 mm coupon left rail centre'),
    ('coupon_rail_center_030', 'coupon_slot_030 / 2 + coupon_wall / 2', 'Print coupons', '0.3 mm coupon rail centre magnitude'),
    ('coupon_rail_center_030_left', '-coupon_rail_center_030', 'Print coupons', '0.3 mm coupon left rail centre'),
    ('coupon_rail_center_040', 'coupon_slot_040 / 2 + coupon_wall / 2', 'Print coupons', '0.4 mm coupon rail centre magnitude'),
    ('coupon_rail_center_040_left', '-coupon_rail_center_040', 'Print coupons', '0.4 mm coupon left rail centre'),
    ('coupon_base_center_y', '-coupon_length / 2 + coupon_wall / 2', 'Print coupons', 'Fit-coupon closed-end centre'),
    ('coupon_base_width_020', 'coupon_slot_020 + 2 * coupon_wall', 'Print coupons', '0.2 mm coupon overall width'),
    ('coupon_base_width_030', 'coupon_slot_030 + 2 * coupon_wall', 'Print coupons', '0.3 mm coupon overall width'),
    ('coupon_base_width_040', 'coupon_slot_040 + 2 * coupon_wall', 'Print coupons', '0.4 mm coupon overall width'),
    ('coupon_faceplate_outer', '52 mm', 'Print coupons', 'Compact Faceplate corner-section outer size'),
    ('coupon_faceplate_inner', 'coupon_faceplate_outer - 2 * bezel_width', 'Print coupons', 'Corner-section rear opening'),
    ('coupon_faceplate_radius', 'device_corner_radius + bezel_width', 'Print coupons', 'Representative outside corner radius'),
    ('coupon_guide_section_width', '32 mm', 'Print coupons', 'Guide and shelf section width'),
    ('coupon_guide_center_x', 'coupon_guide_section_width / 2 + dock_side_wall / 2', 'Print coupons', 'Right guide-section rail centre'),
    ('coupon_guide_center_x_left', '-coupon_guide_center_x', 'Print coupons', 'Left guide-section rail centre'),
    ('coupon_shelf_width', 'coupon_guide_section_width + 2 * dock_side_wall', 'Print coupons', 'Guide-section shelf width'),
    ('coupon_shelf_center_y', '-coupon_length / 2 + lower_support_thickness / 2', 'Print coupons', 'Guide-section lower shelf centre'),
    ('coupon_wall_panel_width', '70 mm', 'Print coupons', 'Wall-stack coupon width'),
...
def _build_faceplate(design, root):
    ...
    component.description = 'FULL SIZE PRINT CANDIDATE ONLY - not a released production part.'
...
def _build_clearance_coupon(design, root, clearance_code):
    """Build a small U-channel that gauges clearance on both tablet faces."""
    component = _new_component(root, 'Coupon_Clearance_' + clearance_code)
    component.description = (
        'PRINT COUPON: '
        + clearance_code[0]
        + '.'
        + clearance_code[1:]
        + ' mm clearance per side; print at 100 percent scale.'
    )
    right_center = 'coupon_rail_center_' + clearance_code
    left_center = right_center + '_left'
    base_width = 'coupon_base_width_' + clearance_code
    for side, center_x in (('Right rail', right_center), ('Left rail', left_center)):
        _add_coupon_box(
            component,
            design,
            side,
    ...
    _add_coupon_box(
        ...
        base_width,
        'coupon_wall',
        'coupon_slot_center_x',
        'coupon_base_center_y',
        'coupon_height',
    )
    return component


def _build_faceplate_coupon(design, root):
    component = _new_component(root, 'Coupon_Faceplate_Corner_Lip')
    component.description = 'PRINT COUPON: representative Rev A rounded corner, bezel, skirt, and screen lip.'
    outer = _mm(design, 'coupon_faceplate_outer')
    inner = _mm(design, 'coupon_faceplate_inner')
    radius = _mm(design, 'coupon_faceplate_radius')
    lip = _mm(design, 'inner_lip_overlap')

    skirt_plane = _offset_plane(
        component,
        'screen_recess - front_thickness',
        'Coupon skirt rear plane',
    )
    skirt_sketch = component.sketches.add(skirt_plane)
    skirt_sketch.name = 'Representative rounded corner skirt'
    _rounded_rectangle(skirt_sketch, outer, outer, radius, 'coupon_faceplate_outer', 'coupon_faceplate_outer', 'coupon_faceplate_radius')
    _rounded_rectangle(skirt_sketch, inner, inner, max(radius - _mm(design, 'bezel_width'), 0), 'coupon_faceplate_inner', 'coupon_faceplate_inner', 'coupon_faceplate_radius - bezel_width')
    skirt = _extrude(component, _ring_profile(skirt_sketch), 'front_thickness - screen_recess')
    skirt.name = 'Representative Faceplate corner skirt'
    skirt.bodies.item(0).name = 'Faceplate corner skirt coupon'

    lip_sketch = component.sketches.add(component.xYConstructionPlane)
    lip_sketch.name = 'Representative visible lip'
    _rounded_rectangle(lip_sketch, outer, outer, radius, 'coupon_faceplate_outer', 'coupon_faceplate_outer', 'coupon_faceplate_radius')
    _rounded_rectangle(lip_sketch, inner - (2 * lip), inner - (2 * lip), max(radius - _mm(design, 'bezel_width') - lip, 0), 'coupon_faceplate_inner - 2 * inner_lip_overlap', 'coupon_faceplate_inner - 2 * inner_lip_overlap', 'coupon_faceplate_radius - bezel_width - inner_lip_overlap')
    lip_feature = _extrude(component, _ring_profile(lip_sketch), 'screen_recess')
    lip_feature.name = 'Representative Faceplate screen lip'
    lip_feature.bodies.item(0).name = 'Faceplate visible lip coupon'
    return component


def _build_guide_shelf_coupon(design, root):
    component = _new_component(root, 'Coupon_Side_Guide_Lower_Shelf')
    component.description = 'PRINT COUPON: representative open insertion section with both guides and lower shelf.'
    for side, center_x in (
        ('Right side guide', 'coupon_guide_center_x'),
        ('Left side guide', 'coupon_guide_center_x_left'),
    ):
        _add_coupon_box(component, design, side, 'dock_side_wall', 'coupon_length', center_x, 'coupon_center_y', 'guide_depth')
    _add_coupon_box(
        component,
        design,
        'Lower support shelf',
...
def _build_wall_stack_coupon(design, root):
    ...
    wall_plane = _offset_plane(
        ...
        'Coupon wall rear plane',
    )
    _add_coupon_box(component, design, 'Wall witness panel', 'coupon_wall_panel_width', 'coupon_wall_panel_height', 'dock_center_x', 'coupon_center_y', 'coupon_wall', wall_plane)
    field_plane = _offset_plane(component, '-wall_shadow_gap', 'Coupon mounting field plane')
    for side, center_x in (
...
def _build_print_coupons(design, root):
    coupons = [
        _build_clearance_coupon(design, root, code)
        for code in ('020', '030', '040')
    ]
    coupons.extend(
        (
            _build_faceplate_coupon(design, root),
            _build_guide_shelf_coupon(design, root),
            _build_wall_stack_coupon(design, root),
        )
    )
    return coupons
...
def _export_part(export_manager, output_dir, basename, component, suffix):
    """Export both editable neutral geometry and the vendor print mesh."""
    step_options = export_manager.createSTEPExportOptions(
        os.path.join(output_dir, basename + '_' + suffix + '.step'),
        component,
    )
    export_manager.execute(step_options)
    stl_options = export_manager.createSTLExportOptions(
        component,
        os.path.join(output_dir, basename + '_' + suffix + '.stl'),
    )
    stl_options.meshRefinement = adsk.fusion.MeshRefinementSettings.MeshRefinementHigh
    export_manager.execute(stl_options)


def _export_outputs(design, faceplate, dock_body, coupons):
    ...
    basename = 'HALO_Dock_Rev_A_External_Print_Candidate'
    ...
    _export_part(export_manager, output_dir, basename, faceplate, 'Faceplate_PRINT_CANDIDATE')
    _export_part(export_manager, output_dir, basename, dock_body, 'DockBody_PRINT_CANDIDATE')
    coupon_suffixes = (
        'Coupon_Clearance_0p2mm_Per_Side',
        'Coupon_Clearance_0p3mm_Per_Side',
        'Coupon_Clearance_0p4mm_Per_Side',
        'Coupon_Faceplate_Corner_Lip',
        'Coupon_Side_Guide_Lower_Shelf',
        'Coupon_Wall_Stack_Shadow_Gap',
    )
    if len(coupons) != len(coupon_suffixes):
        raise RuntimeError('Coupon component count does not match the controlled export manifest.')
    for component, suffix in zip(coupons, coupon_suffixes):
        _export_part(export_manager, output_dir, basename, component, suffix)
...
def run(context):
    try:
        ...
        coupons = _build_print_coupons(design, root)
        _build_assembly_placeholder(root)
        output_dir = _export_outputs(design, faceplate, dock_body, coupons)
        if UI:
            UI.messageBox('HALO Dock Rev A external print candidate generated and exported to:\n' + output_dir)
    except Exception:
        if UI:
